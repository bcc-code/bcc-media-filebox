...
WHERE provider != 'guest' AND lower(email) = lower(?1)
ORDER BY last_login_at DESC
LIMIT 1
`

// Used by guest sign-up to reject emails already claimed by an OAuth identity.
func (q *Queries) GetNonGuestUserByEmail(ctx context.Context, email string) (User, error) {
	row := q.db.QueryRowContext(ctx, getNonGuestUserByEmail, email)
	var i User
	err := row.Scan(
		&i.ID,
		&i.Provider,
...
SELECT id, provider, subject, email, name, created_at, last_login_at, role FROM users WHERE id = ?1
`
...
SELECT id, provider, subject, email, name, created_at, last_login_at, role FROM users WHERE lower(email) = lower(?1) ORDER BY last_login_at DESC LIMIT 1
`
...
SELECT id, provider, subject, email, name, created_at, last_login_at, role FROM users WHERE provider = ?1 AND subject = ?2
`

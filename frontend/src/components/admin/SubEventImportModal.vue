...
const parsed = computed<{ rows: Row[]; trailing: string | null }>(() => {
  ...
  const existingNames = new Set(
    props.arrangement.subEvents.map((s) => s.name.toLowerCase()),
  )
  const existingCodes = new Set(
    props.arrangement.subEvents.map((s) => s.code.toLowerCase()),
  )
  ...
  const rows: Row[] = pairs.map(({ name, code }) => {
    ...
    if (!name) error = 'Name is missing'
      ...
      error = "Code may contain only letters, digits, '-' and '_'"
    else if (seenNames.has(name.toLowerCase()))
      error = 'Duplicate name in the pasted list'
    else if (seenCodes.has(code.toLowerCase()))
      error = 'Duplicate code in the pasted list'
...
const newCount = computed(
  () => parsed.value.rows.filter((r) => r.status === 'new').length,
)
const skipCount = computed(
  () => parsed.value.rows.filter((r) => r.status === 'skip').length,
)
const hasErrors = computed(
  () =>
    parsed.value.trailing !== null ||
    parsed.value.rows.some((r) => r.status === 'error'),
)
const canImport = computed(
  () => !hasErrors.value && newCount.value > 0 && !importing.value,
)

async function onImport() {
  if (!canImport.value) return
  importing.value = true
  const items = parsed.value.rows
...
<template>
  <UiDialog title="Import sub events" @close="emit('close')">
    <template #description>
      Paste a list for “{{ arrangement.name }}”: one line with the sub event
      name followed by one line with its code (or name and code separated by a
      tab). Blank lines are ignored; entries that already exist are skipped.
    </template>
    ...
    <div v-if="parsed.rows.length > 0 || parsed.trailing" class="preview">
      ...
      >
        ...
        <span class="prev-status">
          ...
          <template v-else-if="r.status === 'skip'"
            >already exists — skipped</template
          >
    ...
    <template #actions>
      <span v-if="parsed.rows.length" class="summary">
        {{ newCount }} new<template v-if="skipCount">
...
.prev-row.skip .prev-status {
  color: var(--color-ink-3);
  font-style: italic;
}
.prev-row.error .prev-status {
  color: var(--color-danger);
}
.prev-row.new .prev-status {
  color: var(--color-ok);
}
.summary {
  margin-right: auto;

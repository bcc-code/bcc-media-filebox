...
<template>
  <UiDialog
    :title="isEdit ? 'Edit arrangement' : 'New arrangement'"
    description="The display name is what uploaders pick; the code is embedded in the resulting filename. Manage its sub events from the arrangement row."
    @close="emit('cancel')"
  >
    <div class="field">
      ...
      <input
        v-model="draft.name"
        placeholder="e.g. Sommerstevne 2026"
        autofocus
      />
    </div>

...
function selectOptions(field: {
  ...
  const opts = optionsFor(field.key, field.optionsSource, field.options).map(
    (o) => ({
      value: o.code,
      label: o.label,
    }),
  ...
  return field.placeholder
    ? [{ value: '', label: field.placeholder }, ...opts]
    : opts
}
      ...
      <div v-for="field in form.fields" :key="field.key" class="field">
        ...
        <template v-else>
          <input
            ...
            :class="{
              'inp-warn': tooShort(field, modelValue[field.key] ?? ''),
            }"
            @input="
          ...
          <datalist v-if="field.suggest" :id="`${form.key}-${field.key}-list`">
            <option
              v-for="s in suggestions?.[field.key] ?? []"
              :key="s"
              :value="s"
            />
          </datalist>
          <span
            v-if="tooShort(field, modelValue[field.key] ?? '')"

...
onMounted(async () => {
  ...
  if (!selected.value && packages.value.length)
    selected.value = packages.value[0].packageId
})
...
<template>
  <div class="preview-bar">
    ...
    <div v-if="packages.length" style="width: 260px">
      <UiSelect
        v-model="selected"
        :options="packages.map((p) => ({ value: p.packageId, label: p.name }))"
        aria-label="Package to preview"
      />
    </div>
  </div>

  <div v-if="!packages.length" class="empty">
    Send a package first to preview it here.
  </div>
  ...
  <div v-else class="public-stage">
    <div class="public-card fb-fade" style="text-align: center">
      ...
      <template v-else-if="preview">
        <div class="public-brand" style="justify-content: center">

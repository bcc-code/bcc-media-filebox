...
<template>
  <div class="verify-icon">
    <svg
      v-if="verificationMethod === 'password'"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
    ...
    <svg
      v-else-if="verificationMethod === 'bcc_login'"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
  ...
  <template v-else-if="verificationMethod === 'bcc_login'">
    ...
    <p class="verify-p">
      This package requires you to be signed in with a <b>BCC Login</b> account
      to access it.
    </p>
    <div v-if="error" class="verify-error">{{ error }}</div>
    <UiButton size="lg" variant="primary" block @click="emit('signInBcc')"
      >Continue with BCC Login</UiButton
    >
  </template>
  ...
  <template v-else>
    ...
    <p class="verify-p">
      This package uses a verification method that isn't supported yet.
    </p>
  </template>
</template>

...
| `UiDrawer`       | `@zag-js/dialog`        | the hand-rolled shell in `UserDrawer.vue`                                                | Closed on `@click.self` and nothing else: no Escape, no `role`/`aria-modal`, no focus trap, no focus restore, no scroll lock — you could tab straight out of it into the page behind. Its shell CSS was `.admin-root`-prefixed, so the drawer was silently admin-only styling. Teleports to `<body>`; the drawer's own CSS moved into `UserDrawer.vue` and the shared list vocabulary into `components.css` to make that possible — see docs/frontend-styling.md.                                                                                                                                                                                                                                                                                                        |
| `TargetSelector` | `@zag-js/radio-group`   | its own card grid, rebuilt on `UiRadioGroup`                                             | Was `<button>`s with `aria-pressed`: no `radiogroup`, no arrow keys. Now one tab stop (via native radio `name` grouping, not roving tabindex) with arrows selecting as they move. Became a 40-line adapter mapping `TargetInfo[]` to options; `UiRadioGroup` gained `layout="grid"`, which tiles the cards and moves the control onto a top row beside the icon. Kept the old look rather than inheriting the stacked card's: `UiRadioGroup` gained `indicator="check"` (a filled accent circle with a tick, opt-in — the verification picker keeps the dot) and grid-card labels are weight 600, which holds up against a name that wraps. The tick uses `--color-accent-ink` like `.btn-primary`, so it is dark-on-accent where the old production build had it white. |
...
Surveyed after the primitives pass; `UiDrawer` and `TargetSelector` are now
done. What is left:
...
- **A radio group is one tab stop natively, not by roving tabindex.** Every
  hidden input keeps `tabindex="0"`; they share a generated `name`, so the
  browser makes the group a single stop. Counting focusable inputs looks like
  four tab stops and invites "fixing" something that is already correct —
  `UiTabs` is the one that genuinely roves.
